	...
	public View getView(int position, View view, ViewGroup parent) {
		...
		row.setOnLongClickListener(new View.OnLongClickListener() {
			...
			public boolean onLongClick(View arg0) {
				try {
					isLongClick = true;
					Toast toast = Toast.makeText(context, "long click....",
							Toast.LENGTH_SHORT);
					toast.show();
		...
		if (stock.equals("In Stock")) {
			...
			row.stock.setText("Out of Stock");
		}

		/*
		 * URL newurl = null; try { newurl = new
		 * URL("http://127.0.0.1:8080/img/"+bookid+".jpg");
		 * row.img.setImageBitmap
		 * (BitmapFactory.decodeStream(newurl.openConnection()
		 * .getInputStream())); } catch (MalformedURLException e) {
		 * 
		 * } catch (IOException e) {
		 * 
		 * }
	...
	class Row extends abLyt {
		...
		public Row(Context context, int width, int height) {
			...
			view.setLayoutParams(new AbsoluteLayout.LayoutParams(
					MainActivity.cf.w, MainActivity.cf.dpix[110], 0, 0));
			addView(view);

			img = new ImageView(context);
			img.setLayoutParams(new LayoutParams(MainActivity.cf.dpix[100],
					MainActivity.cf.dpix[71], MainActivity.cf.dpix[15],
					MainActivity.cf.dpix[15]));
			addView(img);
