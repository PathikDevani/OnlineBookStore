...
public class AddBookLayout extends abLyt {
	...
	abLyt cancle,ok;
	public AddBookLayout(final Context context, final int width, int height, int x, int y) {
		...
		cancle = new abLyt(context, (width/2) - MainActivity.cf.dpix[10], MainActivity.cf.dpix[40], MainActivity.cf.dpix[5], height - MainActivity.cf.dpix[45]){
			{
				setBackgroundColor(Color.RED);
				TextView title = new TextView(context);
				setTXT(title, "Cancle", (width/2) - MainActivity.cf.dpix[10], MainActivity.cf.dpix[40]);
				addView(title);
			}
			@Override
		...
		ok = new abLyt(context, (width/2) - MainActivity.cf.dpix[10], MainActivity.cf.dpix[40], (width/2) + MainActivity.cf.dpix[5], height - MainActivity.cf.dpix[45]){
			{
				
				setBackgroundColor(Color.GREEN);
				TextView title = new TextView(context);
				setTXT(title, "Ok", (width/2) - MainActivity.cf.dpix[10], MainActivity.cf.dpix[40]);
			...
			public void onClick(MotionEvent event) {
				...
				if(bookprice.equals("")||bookpublisher.equals("")||booklangugage.equals("")||bookname.equals("")||bookauthor.equals("")||bookpage.equals("")||bookitem.equals("")||bookseller.equals("")){
					...
					if(keyList.brodCast.isServerStart){
						...
						Book b = AppService.db.getLastBook();
						i.putExtra("bid", b.getId());
